...
constexpr int REF_SOFT = 0;
constexpr int REF_OPENGL = 1;
constexpr int REF_VULKAN = 2;
constexpr int REF_3DFX = 3;
constexpr int REF_POWERVR = 4;
constexpr int REF_VERITE = 5;
    ...
    if (stricmp(vid_ref->string, "gl") == 0)
    {
        if (vid_gamma->modified)
        {
            ...
            if (stricmp(gl_driver->string, "3dfxgl") == 0)
            {
                ...
                envbuffer = std::format("SSTV2_GAMMA={}", g);
                putenv(envbuffer.data());
                envbuffer = std::format("SST_GAMMA={}", g);
                putenv(envbuffer.data());
...
void VID_MenuInit(void)
{
    static const char *resolutions[] = {
        "[custom   ]", "[320 240  ]", "[400 300  ]", "[512 384  ]", "[640 480  ]", "[800 600  ]",
        "[960 720  ]", "[1024 768 ]", "[1152 864 ]", "[1280 960 ]", "[1366 768 ]", "[1600 1200]",
        "[1920 1080]", "[1920 1200]", "[2048 1536]", "[2560 1440]", "[3840 2160]", 0};
    static const char *refs[] = {"[software      ]", "[default OpenGL]", "[Vulkan        ]", "[3Dfx OpenGL   ]",
                                 "[PowerVR OpenGL]",
                                 //		"[Rendition OpenGL]",
                                 0};
    static const char *yesno_names[] = {"no", "yes", 0};
    static const char *msaa_modes[] = {"no", "x2", "x4", "x8", "x16", 0};
    static const char *filter_modes[] = {"nearest", "linear", "mipmap nearest", "mipmap linear", 0};
    int i;
    ...
    if (std::string(vk_texturemode->string) == "VK_LINEAR")
        s_texture_filter.curvalue = 1;
    if (std::string(vk_texturemode->string) == "VK_MIPMAP_NEAREST")
        s_texture_filter.curvalue = 2;
    if (std::string(vk_texturemode->string) == "VK_MIPMAP_LINEAR")
    ...
    if (std::string(vk_lmaptexturemode->string) == "VK_LINEAR")
        s_lmap_texture_filter.curvalue = 1;
    if (std::string(vk_lmaptexturemode->string) == "VK_MIPMAP_NEAREST")
        s_lmap_texture_filter.curvalue = 2;
    if (std::string(vk_lmaptexturemode->string) == "VK_MIPMAP_LINEAR")
...
void VID_MenuDraw(void)
{
    ...
    ** draw the banner
    */
    Draw_GetPicSize(&w, &h, std::string("m_banner_video").data());
    Draw_Pic(viddef.width / 2 - w / 2, viddef.height / 2 - 110 * vid_hudscale->value,
             std::string("m_banner_video").data());
