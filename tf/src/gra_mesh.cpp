                           ...
                           int translucentIdx)
{
    ...
    Pipeline *pipelines[2][4] = {
        {
            drawModelPipelineStrip[pidx],
            drawModelPipelineFan[pidx],
            drawLefthandModelPipelineStrip,
            drawLefthandModelPipelineFan,
        },
        {
            drawNoDepthModelPipelineStrip,
            drawNoDepthModelPipelineFan,
            drawLefthandModelPipelineStrip,
            drawLefthandModelPipelineFan,
        },
    };
    for (int p = 0; p < 2; p++)
    {
        ...
        cmdBindDescriptorSet(pCmd, 0, pDSTextureModel[skin->index]);

        if (p == TRIANGLE_STRIP)
        {
            for (i = 0; i < pipeCounters[p]; i++)
            {
                cmdDraw(pCmd, drawInfo[p][i].vertexCount, drawInfo[p][i].firstVertex);
            }
        }
        else
        {
...
void Vk_DrawAliasShadow(dmdl_t *paliashdr, int posenum, float *modelMatrix)
{
    ...
    int i;
    daliasframe_t *frame;
    Pipeline *pipelines[2] = {shadowsPipelineStrip, shadowsPipelineFan};
...
static qboolean R_CullAliasModel(vec3_t bbox[8], entity_t *e)
{
    ...
    {
        LOGF(eWARNING, "R_CullAliasModel %s: no such frame %d\n", currentmodel->name, e->frame);
        e->frame = 0;
    }
    if ((e->oldframe >= paliashdr->num_frames) || (e->oldframe < 0))
    {
        LOGF(eWARNING, "R_CullAliasModel %s: no such oldframe %d\n", currentmodel->name, e->oldframe);
        e->oldframe = 0;
    }
...
void R_DrawAliasModel(entity_t *e)
{
    ...
    {
        LOGF(eWARNING, "R_DrawAliasModel %s: no such frame %d\n", currentmodel->name, currententity->frame);
    ...
    {
        LOGF(eWARNING, "R_DrawAliasModel %s: no such oldframe %d\n", currentmodel->name, currententity->oldframe);
        currententity->frame = 0;
        currententity->oldframe = 0;
    }
    ...
    Vk_DrawAliasFrameLerp(paliashdr, currententity->backlerp, skin, model, leftHandOffset,
                          currententity->flags & RF_TRANSLUCENT ? 1 : 0);
