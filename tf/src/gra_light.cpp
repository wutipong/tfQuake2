...
void R_RenderDlight(dlight_t *light)
{
    ...
    for (i = 16; i >= 0; i--)
    {
        ...
        for (j = 0; j < 3; j++)
        {
            lightVerts[i + 1].verts[j] = light->origin[j] + vright[j] * cos(a) * rad + vup[j] * sin(a) * rad;
...
void R_BuildLightMap(msurface_t *surf, byte *dest, int stride)
{
    ...
    {
        LOGF(eERROR, "R_BuildLightMap called for non-lit surface");
    }
    smax = (surf->extents[0] >> 4) + 1;
    tmax = (surf->extents[1] >> 4) + 1;
    size = smax * tmax;
    ...
    {
        LOGF(eERROR, "Bad s_blocklights size");
    }
