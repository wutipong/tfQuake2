...
#line 87 "D:/Documents/tfQuake2/tf/shaders/ShaderList.fsl"
#line 1 "D:/Documents/tfQuake2/tf/shaders/world_warp.frag.fsl"


PUSH_CONSTANT(PushConstant, b1)
{
#define _Gettime PushConstant.time
#line 5
	DATA(float, time, None);
#define _Getscale PushConstant.scale
#line 6
	DATA(float, scale, None);
#define _GetscrWidth PushConstant.scrWidth
#line 7
	DATA(float, scrWidth, None);
#define _GetscrHeight PushConstant.scrHeight
#line 8
...
//    INIT_MAIN;
	...
	if (Get(time) > 0)
	{
		...
		float xShift = 2.0 * Get(time) + uv.y *  3.1415  * 10;
		float yShift = 2.0 * Get(time) + uv.x *  3.1415  * 10;
	...
	{
		out_float4 = SampleTex2D(Get(sTexture), Get(textureSampler), uv);
		return;
	}
#line 38
//	RETURN(SampleTex2D(Get(sTexture), Get(textureSampler), uv));
}
#line 88 "D:/Documents/tfQuake2/tf/shaders/ShaderList.fsl"
