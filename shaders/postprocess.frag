...
#line 67 "D:/Documents/tfQuake2/tf/shaders/ShaderList.fsl"
#line 1 "D:/Documents/tfQuake2/tf/shaders/postprocess.frag.fsl"
PUSH_CONSTANT(PushConstant, b1)
{
#define _Getpostprocess PushConstant.postprocess
#line 3
	DATA(float, postprocess, None);
#define _Getgamma PushConstant.gamma
#line 4
...
//    INIT_MAIN;
	...
	if (Get(postprocess) > 0.0)
	{
		...
		{
			out_float4 = float4(pow(SampleTex2D(Get(sTexture), Get(textureSampler), In.texCoord).rgb * 1.5, float3(Get(gamma), Get(gamma), Get(gamma))), 1.0);
			return;
		}
#line 24
//		RETURN(float4(pow(SampleTex2D(Get(sTexture), Get(textureSampler), In.texCoord).rgb * 1.5, float3(Get(gamma), Get(gamma), Get(gamma))), 1.0));
	...
	{
		{
			out_float4 = SampleTex2D(Get(sTexture), Get(textureSampler), In.texCoord);
			return;
		}
#line 28
...
#line 68 "D:/Documents/tfQuake2/tf/shaders/ShaderList.fsl"
